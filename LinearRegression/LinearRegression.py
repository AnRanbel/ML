...
def linearRegression(alpha=0.01,num_iters=400):
    ...
    X,mu,sigma = featureNormaliza(X)    # 特征归一化
    ...
    # print('y的列向量：',y)
    theta,J_history = gradientDescent(X, y, theta, alpha, num_iters) # 梯度下降
...
def featureNormaliza(X):
    ...
    for i in range(X.shape[1]):     # 遍历列
        ...
        X[..., i] = (X[..., i] - mu[i]) / sigma[i]  # 归一化
    np.savetxt("test_X2.txt",X,delimiter=',')
...
def gradientDescent(X,y,theta,alpha,num_iters):
    ...
    for i in range(num_iters):  # 遍历迭代次数    
        ...
        temp[..., i] = theta - ((alpha / m) * (np.dot(np.transpose(X), h - y)))  # 梯度的计算(同步更新)
        theta = temp[...,i]
